"""Generic command runner for `strix-pentest cloud`.
...
def _execute(  # noqa: PLR0912, PLR0915
    ...
    *,
    ...
    emit(
        ...
        hint=(
            "Switch with `strix-pentest cloud workspaces use NUMBER`."
            if workspace_list
            else (
                "For Git providers, disconnect with `strix-pentest cloud integrations disconnect "
...
def _build_parser(group: str, verb_label: str, cmd: Cmd) -> argparse.ArgumentParser:
    parser = CloudArgumentParser(
        prog=f"strix-pentest cloud {group} {verb_label}", description=cmd.help
    )
    for name in _PLACEHOLDER.findall(cmd.path):
...
def _interrupted_scan_launch_error(idempotency_key: str | None = None) -> http.CloudError:
    ...
    message = (
        ...
        f"`strix-pentest cloud scans list` before retrying.{retry_note}"
    )
    payload: dict[str, Any] = {
        "error": message,
        "interrupted": True,
...
def _ambiguous_scan_launch_error(
    ...
    retry_note = _idempotency_retry_note(idempotency_key)
    message = (
        f"{error} The scan launch outcome is unknown; check "
        "`strix-pentest cloud scans list` before "
